            ...
            <table id="discountsTable" class="table table-striped table-hover table-bordered" style="width:100%">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-sm btn-info" data-id="{{ $d->iddiscount }}" onclick="showProducts(this)">
                                Ver ({{ $d->products->count() ?? 0 }})
                            </button>
                        </td>
                        <td class="text-nowrap">
                            <a href="{{ route('discount.edit', $d->iddiscount) }}" class="btn btn-sm btn-warning m-1">Editar</a>
...
<script>
    $(document).ready(function() {
        $('#discountsTable').DataTable({
            ...
            columnDefs: [
                { orderable: false, targets: [3, 4] } // Deshabilitar ordenamiento en Productos y Acciones
            ]
        });
    });
        ...
        fetch(`/discount/${id}/products`)
            ...
            .then(data => {
                ...
                if (data.products.length === 0) {
                    body.innerHTML = '<p class="text-muted text-center mb-0">No hay productos con esta promoción.</p>';
                } else {
                    let html = '<div class="row">';
                    data.products.forEach(p => {
                        html += `
                            <div class="col-12 col-md-6 d-flex gap-3 mb-3">
                                <img src="${p.pathimg}" width="80" class="border" />
                                <div>
                                    <div class="fw-bold">${p.name}</div>
                                    <div class="text-muted small">${p.brand}</div>
                                    <div class="small">
                                        <del class="text-muted">₡${p.old_price}</del>
                                        <span class="fw-bold text-success ms-1">₡${p.new_price}</span>
                                    </div>
                                </div>
                            </div>
                        `;
